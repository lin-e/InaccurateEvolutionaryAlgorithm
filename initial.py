...
TARGET_WIDTH = 1 # width of the target zone (in metres, technically)
POPULATION = 25 # the size of the population per generation
GENERATIONS = 10000 # the number of generations to simulate
HIT_BOOST = 1.5 # the amount the health is boosted by when it hits the target zone
SPEED_RANGE = (5, 15) # random range for initial population
ANGLE_RANGE = (0, 90) # ''
RANDOM = True
OPT_SPEED = randint(8, 11) if RANDOM else 10 # apparently the average speed of a thrown basketball? 
...
class generation:
    ...
    def simulate(self, c=(0, 0, 0)):
        ...
        for individual in self.population: # each item in population
            ...
            if health < 0: # if it is negative
                health = 0 # set to zero (see, using a negative probability seems slightly pointless)
    ...
    def reproduce(self):
        ...
        for i in range(0, len(self.population)): # same population as current generation
            ...
            new_angle = parent_a.angle if randint(0, 1) == 0 else parent_b.angle # ''
...
for generation_number in range(0, GENERATIONS): # does this per generation
    ...
    print("    Best: [ Speed:", "{0:.4f}".format(last_generation.sorted_health[0].speed),
